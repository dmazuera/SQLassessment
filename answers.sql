...
SELECT * FROM models WHERE brand_id = 'pon' LIMIT 6;
...
SELECT brand_id, name FROM models WHERE year = 1964 LIMIT 9;
...
WHERE models.name = 'Mustang' LIMIT 1;
...
SELECT COUNT(*)/2 FROM models JOIN brands
ON brands.brand_id = models.brand_id
...
SELECT * FROM models
WHERE model_id >= 15 AND model_id <25
ORDER BY year;
...
SELECT DISTINCT b.name,m.name, m.year, b.discontinued, b.discontinued - m.year AS years_until_brand_discontinued FROM models AS m LEFT JOIN brands AS b ON m.brand_id = b.brand_id WHERE b.discontinued IS NOT NULL ORDER BY b.name, m.name, m.year; 
...
SELECT DISTINCT b.name, b.founded FROM brands AS b LEFT JOIN models AS m ON b.brand_id = m.brand_id WHERE m.name IS NULL;